...
{
    char **array = malloc(sizeof(char *) * (my_count_word(str) + 2));
    int i = 0;
    int j = 0;

    my_memset(array, 0, sizeof(char *) * (my_count_word(str) + 2));
    while (str[i]) {
        if (is_alphanum(str[i]) == 0) {
            array[j] = malloc(sizeof(char) * (get_col(&str[i]) + 1));
            my_memset(array[j], 0, sizeof(char) * (get_col(&str[i]) + 1));
